...
This whitepaper documents the tokenomics, the service node structure, the security model, and the platform roadmap. The figures in this document reflect the completed migration to the XEQM Labs mainnet and the verified XEQM supply.

---

## 2. Supply Certainty

The migration is complete. 276,917,604 XEQM were issued on the XEQM Labs mainnet in exchange for legacy holdings, and that figure is the verified, mathematically anchored starting supply of the network.
...
New XEQM enters circulation only through service node block rewards and governance wallet allocations. Both are fixed. Both are public.

### No Mint, No Burn

XEQM Labs commits to a fixed-supply discipline: no discretionary mints, and no token burns. The cryptographic architecture this network is built on does not support provable burns in the way that term is typically used. Any address a coin is sent to is only as unspendable as the key is demonstrably destroyed, and we will not represent any wallet as a burn address. Supply is managed through the verified starting total, predictable emissions, and staking demand. The supply curve a developer, exchange, or operator evaluates today is the supply curve they can rely on going forward.
...
At 200,000 XEQM per full node, 700 nodes lock 140,000,000 XEQM, which is approximately 50.6% of the total supply. Removing that share from active circulation dampens speculative volatility and creates sustained demand for the token. The remaining approximately 137,000,000 XEQM in free float gives exchange markets and the developer onboarding pipeline the depth they need.
